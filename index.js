...
const RAPIDAPI_KEY     = process.env.RAPIDAPI_KEY     || process.env.RapidAPI     || process.env.RAPIDAPI;     // AeroDataBox
const FOOTBALL_KEY     = process.env.FOOTBALL_KEY     || process.env.FootballKey  || process.env.FOOTBALL;     // football-data.org
const TICKETMASTER_KEY = process.env.TICKETMASTER_KEY || process.env.ticketmaster || process.env.Ticketmaster; // ticketmaster
...
// ─── PEAK HOUR DETECTION (day-specific) ──────────────────────────────────────
// Sido's custom peak schedule:
//   Mon-Thu: 05-09, 15:30-19:30, 20-22
//   Fri:     05-09, 15-19:30, 20-03:30 (next day)
//   Sat:     10-14, 18-22, 22-04 (next day)
//   Sun:     00-04, 16-21
// Note: ranges that span midnight are split between two days.
function isPeakHour() {
  const now = new Date();
  ...
  // Sunday (0): 00:00-04:00 (own range) + 16:00-21:00 + late-Sat overflow 00:00-04:00 (same period)
  if (day === 0) {
    if (hm >= 0 && hm < 4)  return true;   // late Sat→Sun overflow + Sunday early morning
    if (hm >= 16 && hm < 21) return true;
    return false;
  }
  // Monday (1): overflow from Fri overnight 00:00-03:30? No - Fri overflow ends Sat morning.
  // Mon-Thu normal pattern:
  if (day >= 1 && day <= 4) {
  ...
  if (day === 5) {
    ...
    if (hm >= 20 && hm < 24)   return true;   // 20:00 - midnight
    return false;
  }
  // Saturday (6):
  if (day === 6) {
    if (hm >= 0  && hm < 3.5)  return true;   // Friday overflow into Sat morning
    if (hm >= 10 && hm < 14)   return true;
    if (hm >= 18 && hm < 24)   return true;   // 18-22 + 22-04 merged = 18-midnight
    return false;
  }
  return false;
}
...
async function fetchArrivals(airportCode) {
  ...
  const url = `https://aerodatabox.p.rapidapi.com/flights/airports/iata/${airportCode}/${fmt(past)}/${fmt(now)}?direction=Arrival&withCancelled=false&withCodeshared=false&withCargo=false&withPrivate=false&withLocation=false`;
  try {
    const r = await fetch(url, {
    ...
    if (!r.ok) {
      ...
      // Common cases:
      //   401/403 = key not subscribed / wrong key
      //   429    = monthly quota exhausted
      //   404    = wrong endpoint / airport
      return [];
    }
    const d = await r.json();
...
// ─── TRAFFIC SCAN — runs during peak hours only ─────────────────────────────
  ...
  for (const city of CITIES) {
    try {
      ...
      if (isHigh && wasLow) {
        ...
        // Hottest street = the slowest one (most surge)
        const hottest = data.streets[0]; // already sorted by pct ascending (worst first)
        const jamStreets = data.streets.filter(r => r.status === "JAM" || r.status === "SLOW");
        const jams = jamStreets.slice(0,4)
                     .map(r => `  📍 ${r.name} — ${r.speed}km/h (${r.pct}%)`).join("\n");
        const rain = data.weather.rain ? `\n🌧 مطر: ${data.weather.mm}mm` : "";
        // Lead with the hottest district so user immediately knows where to go
        const hotLine = (hottest && (hottest.status === "JAM" || hottest.status === "SLOW"))
...
// Toyota Corolla Hybrid 2024 — service intervals
const MAINT = {
  oilChange:     15000,  // كل 15,000 كم: زيت + فلتر زيت
  airFilter:     30000,  // كل 30,000 كم: فلتر هواء + مكيف + تدوير إطارات
  fullCheck:     60000,  // كل 60,000 كم: قير + بطارية + هايبرد + بواجي
};

// Helper: get latest odometer entry
function getLatestKm() {
  const d = loadData();
  if (!d.kmLog || !d.kmLog.length) return null;
  return d.kmLog[d.kmLog.length - 1]; // {km, date}
}
...
async function handleService(msg, type, label) {
  ...
  d.serviceLog.push({ type, km: last.km, date: new Date().toISOString() });
  saveData(d);
...
// تذكير أسبوعي يوم الأربعاء الساعة 12:00
...
function scheduleWeeklyMaintenance() {
  const scheduleNext = () => {
    ...
    setTimeout(async () => {
      try { await weeklyMaintenanceReminder(); }
      catch(e) { console.error(`Maintenance reminder: ${e.message}`); }
      scheduleNext();
    }, delay);
  };
  scheduleNext();
...
// Seed initial maintenance state on first launch (idempotent — only if empty)
function seedMaintenance() {
  const d = loadData();
  if (d.kmLog.length > 0) return; // already seeded
  // From Sido: car is at 162,941 km today (May 9, 2026).
  // Oil + oil filter changed today.
  // Air filter + cabin filter changed at 147,941 km (15K ago).
  d.kmLog.push({ km: 162941, date: new Date().toISOString() });
  d.lastServiceKm.oilChange = 162941;        // changed today
  d.lastServiceKm.airFilter = 147941;        // changed 15K ago (so next due at 177,941)
  d.serviceLog.push(
...
// Traffic scan — every 10 minutes (continuous, no peak hour filter)
...
scheduleDailyReport();
// Weekly maintenance reminder (every Wednesday 12:00)
scheduleWeeklyMaintenance();
// Seed initial maintenance state (only runs once on first launch)
seedMaintenance();
