...
if (!m) {
await bot.sendMessage(msg.chat.id, `❌ مثال: \`${type} 60``, { parse_mode:"Markdown" }); return; } const amount = parseFloat(m[1].replace(",", ".")); const data = loadData(); data.expenses.push({ type, amount, date: new Date().toISOString(), }); saveData(data); await bot.sendMessage(msg.chat.id, `✅ تم تسجيل ${label}: ${fmt(amount)}`);
}
