...
text += ` اﻹﺟﻤﺎﻟﻲ: ${fmt(S.gesamt)}\n`;
text += ` 16 اﻟﻀﺮﯾﺒﺔ%: -${fmt(salamTax)}\n`;
text += ` ﺻﺎﻓﻲ: *${fmt(salamNet)}*\n\n`;
text += ` *ﻛﺎوا* (${K.fahrten} رﺣﻠﺔ)\n`;
text += ` اﻹﺟﻤﺎﻟﻲ: ${fmt(K.gesamt)}\n`;
text += ` 40 ﺣﺼﺔ ﻛﺎوا%: ${fmt(kawaShare)}\n`;
text += ` 60 ﺣﺼﺘﻚ%: ${fmt(ownerFromKawa)}\n`;
text += ` 16 اﻟﻀﺮﯾﺒﺔ%: -${fmt(kawaTax)}\n`;
text += ` ﺻﺎﻓﻲ ﻟﻚ: *${fmt(ownerFromKawaNet)}*\n\n`;
text += `━━━━━━━━━━━━━━━━━\n`;
text += ` *اﻟﻤﺼﺎرﯾﻒ*\n`;
text += ` ﺷﺮﻛﺔ: ${fmt(FIXED_COMPANY)}\n`;
text += ` ﺗﺄﻣﯿﻦ: ${fmt(FIXED_INSURANCE)}\n`;
text += ` ﺑﻨﺰﯾﻦ: ${fmt(fuel)}\n`;
text += ` ﺗﺼﻠﯿﺢ: ${fmt(repair)}\n`;
text += ` ﻏﺴﯿﻞ: ${fmt(wash)}\n`;
text += ` اﻟﻤﺠﻤﻮع: *${fmt(totalExp)}*\n\n`;
text += `━━━━━━━━━━━━━━━━━\n`;
text += ` *اﻟﻨﺘﯿﺠﺔ اﻟﻨﮭﺎﺋﯿﺔ*\n`;
text += ` دﺧﻠﻚ: ${fmt(grossOwner)}\n`;
text += ` اﻟﻤﺼﺎرﯾﻒ: -${fmt(totalExp)}\n`;
text += ` ════════════════\n`;
text += ` رﺑﺤﻚ اﻟﺼﺎﻓﻲ: *${fmt(netOwner)}*\n`;
text += ` ﻟﻜﺎوا: *${fmt(kawaShare)}*\n`;
...
+ `n\ﺳﻼم* — ﺗﺴﺠﯿﻞ دﺧﻠﻚ )اﻟﺼﻖ اﻟﺘﻘﺮﯾﺮ ﺑﻌﺪ اﻟﻜﻠﻤﺔ(*`
+ `n\ﻛﺎوا* — ﺗﺴﺠﯿﻞ دﺧﻞ ﻛﺎوا*`
+ `n\ﺑﻨﺰﯾﻦ 60* — ﻓﺎﺗﻮرة ﺑﻨﺰﯾﻦ*`
+ `n\ﺗﺼﻠﯿﺢ 250* — ﻓﺎﺗﻮرة ﺗﺼﻠﯿﺢ*`
+ `n\ﻏﺴﯿﻞ 15* — ﻓﺎﺗﻮرة ﻏﺴﯿﻞ*`
+ `n\ﺗﻘﺮﯾﺮ* — اﻟﺘﻘﺮﯾﺮ اﻟﺸﮭﺮي اﻟﻜﺎﻣﻞ*`
+ `n\n\ﻣﺴﺢ اﻟﺒﯿﺎﻧﺎت* — ﯾﻤﺴﺢ ﻛﻞ ﺷﻲ*`
...
واﻟﻤﺼﺎرﯾﻒ* n\n\ﻣﻄﺎرات }AIRPORTS.length{$ · ﻣﺪﯾﻨﺔ }NRW Surge Bot v6*\n${CITIESde:"Markdown" });
*trafficScan();
flightScan();
