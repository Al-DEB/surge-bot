...
catch { return { salam:[], kawa:[], expenses:[], strikes:[], alertedFlights:{}, alertedMatc
}
...
3: { name:"Bayer Leverkusen", venue:"BayArena", cityId:"leverkusen" },
18: { name:"Borussia Mönchengladbach", venue:"Borussia-Park", cityId:"moenchengladbach" },
...
28: { name:"FC Köln", venue:"RheinEnergieStadion", cityId:"koeln" },
...
if (diff > 0 && diff < 2*60*60*1000 + 5*60*1000) {
const key = `pre_${m.id}`;
if (data.alertedMatches[key]) continue;
data.alertedMatches[key] = now;
const city = CITIES.find(c => c.id === m.cityId);
const dist = city ? city.distFromHerne : "?";
const localTime = new Date(m.time).toLocaleString("de-DE", { hour:"2-digit", minute:"2-
await bot.sendMessage(CHAT_ID,
m.away}\n ${localTime}\n ${m.venue}\n{$ ﺿﺪ }n\n${m.home\*ﻣﺒﺎراة ﺑﻌﺪ ﺳﺎﻋﺘﯿﻦ* `
{ parse_mode:"Markdown" });
}
// Alert when match likely just ended (~110 min after start)
const endDiff = now - matchTime;
if (endDiff > 110*60*1000 && endDiff < 115*60*1000) {
...
const dist = city ? city.distFromHerne : "?";
await bot.sendMessage(CHAT_ID,
m.away}\n ${m.venue}\n ${city{$ ﺿﺪ }n\n${m.home\*!اﻟﻤﺒﺎراة اﻧﺘﮭﺖ — اﺗﺠﮫ e_mode:"Markdown" });
}
}
...
text += ` text += ` text += ` 10:00-14:00 — ${top[0]?.city.name || "Düsseldorf"}\n`;
16:00-18:00 — ${top[1]?.city.name || "Dortmund"}\n`;
20:30-23:30 — ${top[0]?.city.name || "Düsseldorf"}\n`;
await bot.sendMessage(CHAT_ID, text, { parse_mode:"Markdown" });
}
// Schedule daily report at 21:00 every day
...
*// Smart traffic scan — every 30 min during peak hours
setInterval(trafficScan, 30 * 60 * 1000);
